...
#   if(is.null(batterData()$df)) return()
  ...
  print("enterobserveEvent(batterData(),{")
  print(glimpse(batterData()$df))
 ...
 # print(str(df))
  ...
  print("pl_strikeRate done")
  df %>% 
    ggvis(~Date,~Runs, key := ~id) %>% 
    layer_points(size = 1, fill= ~Opposition) %>% 
    add_tooltip(all_values, "hover") %>% 
    bind_shiny("pl_batByDateChart")
  print("pl_batByDateChart done")
  print(df$Runs)
  
  write_csv(df,"baigTest.csv")
